        ...
        <tr onclick="viewEvent(${event.event_id})">
            ...
            <td>
                <div style="display: flex; gap: 8px;">
                    <button onclick="event.stopPropagation(); editEvent(${event.event_id})" class="btn-small btn-primary">${icon('edit')} Edit</button>
                    ${event.status !== 'cancelled' ? `
                        <button onclick="event.stopPropagation(); cancelEvent(${event.event_id})" class="btn-small btn-warning">${icon('x')} Cancel</button>
                    ` : ''}
                    <button onclick="event.stopPropagation(); deleteEvent(${event.event_id})" class="btn-small btn-danger">${icon('trash')} Delete</button>
                </div>
            </td>
        </tr>
    `).join('');
...
async function viewEvent(id) {
    try {
        ...
        var viewMod = document.getElementById('viewModal');
        viewMod.style.display = 'flex';
        viewMod.classList.add('active');

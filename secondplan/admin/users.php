...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $jsonAction = $jsonInput['action'] ?? $_POST['action'] ?? null;
}

$isApi = isset($_GET['api']) || $jsonAction || isset($_POST['action']);
...
// CREATE USER
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$jsonAction) {
    try {
        $input = $jsonInput;
...
// UPDATE USER
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $jsonAction === 'update') {
    try {
        $input = $jsonInput;
...
// DELETE USER
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($jsonAction === 'delete' || ($_POST['action'] ?? '') === 'delete')) {
    try {
        $id = (int)($_POST['id'] ?? 0);
...
// RESET PASSWORD
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $jsonAction === 'reset_password') {
    try {
        $input = $jsonInput;
        ...
        <main class="content">
            ...
            <div class="page-header">
                ...
                <button class="btn-primary" onclick="openAddUserModal()">
                    <i class="bi bi-plus-circle btn-icon"></i> Add User
                </button>
            </div>
...
<div class="modal" id="userModal">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <button class="btn-primary" onclick="saveUser()"><i class="bi bi-floppy btn-icon"></i> Save User</button>
        </div>
    </div>
</div>
